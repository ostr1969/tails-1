...
def search_chunks_knn(es_client, chunk_index: str, nchunks: int, query: str, document_ids: str = None) -> List[Dict]:
...
def single_doc_response(document_id, user_query,history=None):
    ...
    messages.append({"role": "user", "content": f"My question is: {user_query}"})
    ...
    return prompt
def rag_query(es_client, chunk_index, nchunks, user_query: str, document_ids: str):
    
   
           
    chunks = search_chunks_knn(es_client, chunk_index, nchunks, user_query, document_ids=document_ids)
